...
var next = document.querySelector('.next'); // const prizes=document.querySelectorAll('.prize li img')
...
prev.addEventListener('click', function () {
  ...
  historyWrap.style.transform = "translateX(".concat(4000 - 2800 - lIndex * 400, "px)");
  changeStyle();
});
...
function changeStyle() {
  var num = lIndex - 1;
  var li = historyWrap.querySelectorAll('li')[num];
  historyWrap.querySelectorAll('li').forEach(function (item) {
    return item.classList.remove('history-last');
  });
...
function historysList() {
  historys.forEach(function (item, index) {
    ...
    if (lIndex === index + 1) {
      li.classList.add('history-last');
      li.innerHTML = "<h3>".concat(item.time, "<span>YEAR</span></h3>\n            <p>").concat(item.content, "</p>");
